...
local SCALERATE = 1 / (30 * 0.1) -- 2 seconds to go from 0 to 1
...
--[[ function RocController:GetTarget() ------------这个函数有大问题
	...
	local ent = FindClosestValidPlayerToInst(bird, 40, true)
	if IsValidPlayer(ent) then
		return ent
	end
...
local function FadeOut(player)
	...
	player:DoTaskInTime(2, function() teleport(player) end)
end
...
function RocController:LoadPostPass(ents, data)
	...
	print(self.stage, "check load stage!!!!!!!!!!!")
	if self.stage > _stages.finding_object then --非该阶段则直接删除
		if self.head then
			self.head:Remove()
			print("removehead!!!")
		end
